...
public class Process extends Agent {
    ...
    public Process(int id, SuperVisor supervisor, List<Operation> operations) throws InterruptedException {
        super(id, supervisor);
        this.operations = operations;
        for (Operation operation : operations) {
            Cooker cooker = AgentRepository.findByType(Cooker.class).stream()
                    .filter(p -> !p.isActive())
                    .findFirst()
                    .orElse(null);
            KitchenEquipment equipment = AgentRepository.findByType(KitchenEquipment.class).stream()
